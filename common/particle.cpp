...
void CParticle::CreateInstance(int count)
{
	if (m_PositionBuffer == nullptr) {
		...
		m_CameraPos = new XMFLOAT4;
		m_CameraPos = &XMFLOAT4(CCamera::GetEye().x, CCamera::GetEye().y, CCamera::GetEye().z, 0.0f);
		...
		CRenderer::GetDeviceContext()->GSSetConstantBuffers(8, 1, &m_CameraPosBuffer);
	}
...
void CParticle::Update()
{
	...
	m_CameraPos = &XMFLOAT4(CCamera::GetEye().x, CCamera::GetEye().y, CCamera::GetEye().z, 0.0f);
	CRenderer::GetDeviceContext()->UpdateSubresource(m_CameraPosBuffer, 0, NULL, m_CameraPos, 0, 0);
